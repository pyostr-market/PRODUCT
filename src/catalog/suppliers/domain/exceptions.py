...
class SupplierNameTooLong(BaseServiceError):
    def __init__(
            self,
            msg: str = "Имя слишком длинное",
            code: str = "supplier_name_too_long",
            status_code: int = 400,
    ):
        super().__init__(message=msg, code=code, status_code=status_code)


class InvalidSupplierName(BaseServiceError):
    def __init__(
            self,
            msg: str = "Некорректное имя поставщика",
            code: str = "invalid_supplier_name",
            status_code: int = 400,
    ):
        super().__init__(message=msg, code=code, status_code=status_code)


class InvalidContactEmail(BaseServiceError):
    def __init__(
            self,
            msg: str = "Некорректный email",
            code: str = "invalid_contact_email",
            status_code: int = 400,
    ):
        super().__init__(message=msg, code=code, status_code=status_code)


class InvalidPhoneNumber(BaseServiceError):
    def __init__(
            self,
            msg: str = "Некорректный телефон",
            code: str = "invalid_phone_number",
            status_code: int = 400,
    ):
        super().__init__(message=msg, code=code, status_code=status_code)
